  ...
  setup() {
    ...
    const selectPost = async (button) =>{
      ...
      if(button === 'finished'){
        ...
        posts.value = data.filter((post) => post.matchState === -1)
      }
      if(button === 'schedule'){
        filterAllData();
        updateScore()
        const data = posts.value;
        posts.value = data.filter((post) => post.matchState === 0)
      }
      loading.value = false;
    }
    ...
    const updateScore = async() =>{
      ...
      posts.value.forEach((post) => {
        ...
        if (liveMatch) {
          post.matchState = liveMatch.matchState;
